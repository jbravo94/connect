...
public class AstmLightLightConnectorServlet extends MirthServlet implements AstmLightConnectorServletInterface {

    protected static final TemplateValueReplacer replacer = new TemplateValueReplacer();

    public AstmLightLightConnectorServlet(@Context HttpServletRequest request, @Context SecurityContext sc) {
        super(request, sc, PLUGIN_POINT);
    }

    @Override
    public ConnectionTestResponse testConnection(String channelId, String channelName, AstmLightDispatcherProperties properties) {
        try {
