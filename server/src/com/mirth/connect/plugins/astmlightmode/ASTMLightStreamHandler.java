...
public class ASTMLightStreamHandler extends FrameStreamHandler {
    ...
    public ASTMLightStreamHandler(InputStream inputStream, OutputStream outputStream, BatchStreamReader batchStreamReader, TransmissionModeProperties transmissionModeProperties) {
        ...
        while (!done) {
            ...
            try {
                ...
                if (Arrays.areEqual(response, ackBytes)) {
                    ...
                    throw new ASTMLightStreamHandlerException("Negative commit acknowledgement received.");
                } else {
                    throw new ASTMLightStreamHandlerException("Invalid acknowledgement block received.");
                }
            } catch (IOException e) {
                if (firstCause == null) {
                    firstCause = e;
                ...
                if (maxRetries > 0 && retryCount++ == maxRetries) {
                    throw new ASTMLightStreamHandlerException("Maximum retry count reached. First cause: " + firstCause.getMessage(), firstCause);
                }
            }
        }
    }
